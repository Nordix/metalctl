...
import (
	...
	"github.com/Nordix/metalctl/pkg/client"
)
...
var configProviderCmd = &cobra.Command{
	...
	Long: LongDesc(`
		metalctl ships with a list of well-known providers; if necessary, edit
	...
	Example: Examples(`
		# Displays relevant information about the metal3 provider, including also the list of 
		# required env variables, if any.
		metalctl config provider

		# Displays relevant information about a specific version of the AWS provider.
		metalctl config provider metal:v1.0

		# Displays the yaml for creating provider components.
		metalctl config provider -o yaml

		# Displays the yaml for creating provider components for a specific version of the AWS provider.
		metalctl config provider aws:v0.4.1 -o yaml`),
...
func runGetRepositories() error {
	...
	repositoryList, err := c.GetProviderConfig()
	if err != nil {
		return err
	}
